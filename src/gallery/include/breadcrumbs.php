...
$breadcrumbs_code .= '<a href="'.$rooturl.'">'.stripslashes($gallerytitle).'</a>';
        ...
        if ($displayfolders[0]!="images"){
			...
			if( file_exists($bcnamefile) ) {
				...
				fclose($fp);
            		...
            		$breadcrumbs_code .= ' - <a href="'.$currenturl.$get_folder.'='.$displayfolders[0].'">'.str_replace("_"," ", RemoveOrderNumber($bc_name) ).'</a>';
	...
	for ($i=1; $i <= sizeof($displayfolders); $i++) {
		...
		if ($displayfolders[$i]!=null) { 
			$breadcrumbs_code .= ' - <a href="'.$currenturl.$get_folder.'='.$displayfolders[0];
			$bc_path = "";
			for ($x=1; $x <= $i; $x++) {
    ...
    if ($getimage!=="") {
	    ...
	    if( in_array( $file_extension , $accepted_img ) || in_array( $file_extension , $accepted_vid ) ) {
			...
			$breadcrumbs_code .= ' - <a href="'.$currenturl.$get_folder.'='.$breadcrumbs.'">';
